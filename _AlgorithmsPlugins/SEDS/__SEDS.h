...
protected:
    ...
    double Compute_J_Constraint(Vector p, Vector& c, Vector& ob);

    void Compute_Constraints(Vector &c);

    void Compute_Constraints(Vector &c, Matrix &dc);

    void optIterate(Vector &p, Vector &lagrangian, Matrix &H, double &mu, double rho, Vector ob, Vector pb);
