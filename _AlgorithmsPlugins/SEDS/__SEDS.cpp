...
bool SEDS::Optimize(){
    ...
    if (Options.objective){
        nPar = Options.perior_opt*K + Options.mu_opt*K*d + Options.sigma_x_opt*K*d*(d+1)  + K*d*d;
        p.Resize(nPar); //a vector of parameters
        GMM_2_Parameters_Likelihood(p);
        scale = 1000.0;
    }else{
        nPar = Options.perior_opt*K + Options.mu_opt*K*d + Options.sigma_x_opt*K*d*(d+1)/2 + K*d*d;
        p.Resize(nPar); //a vector of parameters
 ...
 * The result will be saved in the variable p
*/
bool SEDS::Optimize_SQP(){
    Matrix B(nPar,nPar);
    dc.Resize(nCtr,nPar);
    c.Resize(nCtr); //a vector representing the constraints
    ...
    str2[0] = (char*)"Eigenvalue";
    str2[1] = (char*)"Principal Minor";
    if (Options.display)
    {
        ...
        printf("Using %s and %s as the objective function and the constraint criterion respectively.\n\n",
               str[Options.objective],str2[Options.constraintCriterion]);
    ...
    rho = Options.cons_penalty*10000.0;
    double beta = 0.9;

    double J = Compute_J(p,dJ); //compute dJ and J
    J *= scale;dJ *= scale;
    double J_new=0;
    ...
    if (Options.display){
        ...
        printf("---------------------------------------------------------------------------------------\n");
        printf("      %-3i     %-10.3f    %-10.3f     %-4.3f          %-4s              %-4s\n",0,J/scale,dJ.Norm(),alpha,str[c1_cond < 0],str[c2_cond > 0]);
    }
    displayData.push_back(J/scale);
    PaintData(displayData);

    bool step_ok;

    for(int i=1; i < Options.max_iter; i++){
        step_ok = false;
        ...
        if(c1_cond < 0 && c2_cond> 0){
            for (int j=0; j<Options.maxMinorIter; j++)
            {
                alpha = 3*exp(-0.5*j); //set the step-length
                p_tmp = p+dp*alpha; //change p
                J_new = Compute_J(p_tmp,dJ_new); //get corresponding J and dJ
                J_new *= scale;dJ_new *= scale;

                if( (J_new < J + dp*dJ*c1*alpha && dp*alpha*dJ_new>=dp*dJ*c2) || dJ.Norm()<Options.tol_stopping || fabs(c1_cond)<Options.tol_stopping){
                    step_ok = true;
        ...
        if (step_ok){
            if (Options.display){
                ...
                printf("      %-3i     %-10.3f    %-10.3f     %-4.3f          %-4s              %-4s\n",i,J/scale,dJ.Norm(),alpha,str[c1_cond < 0],str[c2_cond > 0]);
            }

            Compute_Hess(B,dJ_new-dJ,dp*alpha); //update the hessian (result in B)
            p = p + dp*alpha; //update p
            J=J_new;
            displayData.push_back(J_new/scale);
            PaintData(displayData);
            dJ=dJ_new;
        }

        if (dJ.Norm()<Options.tol_stopping || fabs(c1_cond)<Options.tol_stopping || !step_ok){
            bool ctrChech = true;
            for (int ii=0; ii<nCtr; ii++){
                if (c[ii]>0)
                    ctrChech = false;
            }
            if (ctrChech && rho > Options.tol_stopping)
                rho *= beta;
            else
...
bool SEDS::Optimize_SLNOP(){
    Vector ib0(nCtr),pb(nCtr),c_p(nCtrPar);
    ib0.Zero();ib0 += -500;
    c_p.SetSubVector(0,ib0); //a vector containing the value of the constraints and the parameters
    c_p.SetSubVector(nCtr,p);
    pb.Zero(); pb += -0.001;

    double J,J_new,mu,rho;
    Vector t_param(3),lagrangian(nCtr),constraint(nCtr),ob(1+nCtr);
    Matrix H(nCtrPar,nCtrPar);
    t_param.Zero();
    lagrangian.Zero();
    rho = Options.cons_penalty;
    mu = nPar;

    J = Compute_J_Constraint(p,constraint,ob);
    double maxCtr = constraint.Max();
    displayData.push_back(J);

    if ((pb-constraint).Min() > 0)
        c_p.SetSubVector(0,constraint);

    constraint -= c_p.GetSubVector(0,nCtr);

    t_param[1] = constraint.Norm();

    H.Zero();
    for (int i=0; i < nPar+nCtr; i++)
        H(i,i) = 1;
    ...
    Vector c_p_old,ob_old;

    for(int iter=1; iter < Options.max_iter; iter++){
        c_p_old.Set(c_p);
        ob_old.Set(ob);

        optIterate(c_p,lagrangian,H,mu,rho,ob,pb);
        c_p.GetSubVector(nCtr,nPar,p);
        J_new = Compute_J_Constraint(p,constraint,ob);
        if (J_new != J_new || J_new > J){
            c_p = c_p_old;
            ob = ob_old;
            displayData.push_back(J);
            PaintData(displayData);
        ...
        maxCtr = constraint.Max();

        t_param[0] = (J-J_new)/max(fabs(J_new),1.0);
        J = J_new;
        displayData.push_back(J);
        PaintData(displayData);
        ...
        constraint -= c_p.GetSubVector(0,nCtr);
        t_param[2] = constraint.Norm();

        if (t_param[2] < 10 * Options.tol_stopping){
            rho = 0;
            mu = min(mu,Options.tol_stopping);
        }

        if (t_param[2] < 5 * t_param[1])
            rho /= 5;
        else if (t_param[2] > 10 * t_param[1]){
            rho = max(rho,sqrt(Options.tol_stopping));
            rho *= 5;
        }

        if (max(Options.tol_stopping+t_param[0],t_param[1]-t_param[2]) <= 0){
            lagrangian.Zero();
            H.Zero();
            for (int i=0; i < nCtrPar; i++)
                H(i,i) = 1;
        }
        ...
        if (Options.display){
            printf("      %-3i     %-10.3f    %-10.3f ",iter,J,maxCtr);
            if (iter%20 == 0){
                printf("\n\n   Iteration      J         max(c)      \n");
                printf("---------------------------------------------------------------------------------------");
            }
        ...
        cout << endl;
        if (t_param.GetSubVector(0,2).Norm() <= Options.tol_stopping)
            break;
    }
...
void SEDS::optIterate(Vector &c_p, Vector &lagrangian, Matrix &H, double &mu, double rho, Vector ob, Vector pb){
    Vector alp(3), dJ(nPar+nCtr), constraint(nCtr), constraint_new(nCtr),p(nPar),dCdp_i(nCtr), b(nCtr);
    Matrix dC(nCtr,nCtrPar);
    double J,J_new;
    ...
    //computing the derivatives from finite difference
    c_p.GetSubVector(nCtr,nPar,p);
    J = ob[0];
    constraint = ob.GetSubVector(1,nCtr);
    for (int i=0; i<nPar ; i++){
        p[i] += Options.delta;
        J_new = Compute_J_Constraint(p,constraint_new,ob);
        dJ[nCtr+i] = (J_new - J)/Options.delta;
        dCdp_i = constraint_new - constraint;
        dCdp_i /= Options.delta;
        dC.SetColumn(dCdp_i,nCtr+i);
        p[i] -= Options.delta;
    }
    ...
    int ch = -1;
    alp[0] = Options.tol_stopping - constraint.Abs().Max();

    if (alp[0] <= 0){
        ch = 1;

        Vector c_p_tmp(nCtrPar+1),c(nCtrPar+1),dx(nCtrPar+1),ctr_tmp(nCtr),c_tmp(nCtrPar+1),y(nCtr),v(nCtrPar+1);
        Matrix dC_tmp(nCtr,nCtrPar+1),dCdx(nCtrPar+1,nCtr);
        double go = 1,z;
        int minit = 0;

        c_p_tmp.SetSubVector(0,c_p);
        c_p_tmp[nCtrPar] = 1;

        dC_tmp.SetColumnSpace(dC,0);
        dC_tmp.SetColumn(constraint*(-1),nCtrPar);
        ...
        while (go >= Options.tol_stopping){
            minit++;
            c_p_tmp.GetSubVector(0,nCtr,ctr_tmp);
            ctr_tmp = pb - ctr_tmp;
            dx.SetSubVector(0,ctr_tmp);
            dx[nCtrPar] = c_p_tmp[nCtrPar];
            double d_tmp = max(ctr_tmp.Max(),100.0);
            for (int j=0; j<nPar; j++)
                dx[nCtr+j] = d_tmp;

            for (int j=0; j<nPar+nCtr+1; j++)
                dCdx.SetRow(dC_tmp.GetColumn(j)*dx[j],j);

            c_tmp = dx^c;
            y = dCdx.Inverse()*c_tmp;
            v = (c-dC_tmp.Transpose()*y)^dx^dx;

            if (v[nCtrPar]>0){
                z = c_p_tmp[nCtrPar]/v[nCtrPar];
                for (int i = 0; i<nCtr; i++){
                    if (v[i]<0 && (c_p_tmp[i]-pb[i])/v[i] < z){
                        z = (c_p_tmp[i]-pb[i])/v[i];
                    }
                }

                if (z >= c_p_tmp[nCtrPar]/v[nCtrPar])
                    c_p_tmp -= v*z;
                else
                    c_p_tmp -= v*z*0.9;
        ...
        dC_tmp.GetColumnSpace(0,nCtrPar,dC);
        c_p_tmp.GetSubVector(0,nCtrPar,c_p);
        b = dC*c_p;
    }
    ...
    if (ch>0){
        ...
        J = ob[0];
        ob.GetSubVector(1,nCtr,constraint);
    }

    constraint -= c_p.GetSubVector(0,nCtr) + dC*c_p - b;
    J += rho*constraint.Norm2() - lagrangian*constraint;
    ob.SetSubVector(1,constraint);

    int minit = 0;
    double Jm,go,sc1,sc2;
    Vector constraint_m(nCtr),obm(nCtrPar),ydJ(nCtrPar),sx(nCtrPar),dx(nCtrPar),y(nCtr),u(nCtrPar),sob(3);
    Vector ob1(nCtrPar),ob2(nCtrPar),ob3(nCtrPar),c_p1(nCtrPar),c_p2(nCtrPar),c_p3(nCtrPar);
    Matrix H_tmp(nCtrPar,nCtrPar);

    while (minit < Options.maxMinorIter){
        minit++;
        if (ch>0){
            c_p.GetSubVector(nCtr,nPar,p);
            dJ.Zero();
            for (int i=0; i<nPar ; i++){
                p[i] += Options.delta;
                Jm = Compute_J_Constraint(p,constraint_m,obm);
                constraint_m -= c_p.GetSubVector(0,nCtr) + dC*c_p - b;
                Jm += rho*constraint_m.Norm2() - lagrangian*constraint_m;
                dJ[nCtr+i] = (Jm - J)/Options.delta;
                p[i] -= Options.delta;
            }
        }
        ...
        if (minit > 1){
            ...
            if (sc1*sc2>0){
                sx = H*sx;
                H = H - sx.MultTranspose(sx)/sc1 + ydJ.MultTranspose(ydJ)/sc2;
            }
        }

        dx.One();dx *= 0.01;
        for (int i=0; i<nCtr; i++){
            dx[i] = 1.0/(pb[i] - c_p[i] + sqrt(DBL_MIN));
        }
        ...
        while (go <= 0){
            ...
            H_tmp *= mu;
            H_tmp += H;
            H_tmp.Cholesky(H_tmp);
            H_tmp.InverseLowerTriangular(H_tmp);
            ydJ = H_tmp*dJ;

            Matrix tmp = H_tmp*dC.Transpose();
            y = tmp.Inverse()*ydJ;
            u = H_tmp.Transpose()*(tmp*y-ydJ);

            c_p0 = u + c_p;
            go = (pb - c_p0.GetSubVector(0,nCtr)).Min();
            mu *= 3;
        }
        ...
        c_p3.GetSubVector(nCtr,nPar,p);
        sob[2] = Compute_J_Constraint(p,constraint_m,ob3);
        constraint_m -= c_p3.GetSubVector(0,nCtr) + dC*c_p3 - b;
        ob3.SetSubVector(1,constraint_m);
        sob[2] += rho*constraint_m.Norm2() - lagrangian*constraint_m;

        go = 1;

        while (go > Options.tol_stopping){
            alp[1] = (alp[0] + alp[2])/2;
            c_p2 = c_p*(1-alp[1]) + c_p0*alp[1];
            c_p2.GetSubVector(nCtr,nPar,p);
            sob[1] = Compute_J_Constraint(p,constraint_m,ob2);
            constraint_m -= c_p2.GetSubVector(0,nCtr) + dC*c_p2 - b;
            ob2.SetSubVector(1,constraint_m);
            sob[1] += rho*constraint_m.Norm2() - lagrangian*constraint_m;

            Jm = sob.Max();
            if (Jm < J){
                double Jn = sob.Min();
                go = (Jm - Jn)/(J - Jm)*Options.tol_stopping;
            }
        ...
        double Jn = sob.Min();
        if (J <= Jn){
            minit = Options.maxMinorIter;
        }

        double reduce = (J - Jn)/(1.0+fabs(J));
        if (reduce < Options.tol_stopping){
            minit = Options.maxMinorIter;
        }

        if (sob[0]<sob[1]){
            J=sob[0];c_p=c_p1;ob=ob1;
        }else if (sob[2]<sob[1]){
            J=sob[2];c_p=c_p3;ob=ob3;
        }else{
            J=sob[1];c_p=c_p2;ob=ob2;
        }
    }
    lagrangian.Set(y);
}
...
double SEDS::Compute_J(Vector pp, Vector& dJ) //compute the objective function and derivative w.r.t parameters (updated in vector dJ)
{
    ...
    Compute_Constraints(c, dc);

    J += 1000*rho*(c*c);
    dJ /= nData;
    dJ += (dc.Transpose()*c)*2*rho*1000; //calc dJ
    return J;
}
...
double SEDS::Compute_J_Constraint(Vector pp, Vector& c, Vector& ob){
    double J = Compute_J(pp);
    Compute_Constraints(c);
    ob(0) = J;
    ob.SetSubVector(1,c);
    return J;
}

void SEDS::Compute_Constraints(Vector &c, Matrix &dc){
    int i1_tmp;
    double tmp_detB, term;
    int counter_sigma = Options.perior_opt*K + Options.mu_opt*K*d; //the index at which sigma should start
    int counter_A = counter_sigma + Options.sigma_x_opt*K*d*(d+1)/2; //the index at which A should start

    double eps_margin = pow(Options.tol_mat_bias,1.0/d);
    c.Zero();
    dc.Zero();
    ...
    if (Options.constraintCriterion){ //Principal Minor
        ...
        for (int k=0; k<K; k++)//for all states
        {
            ...
            for (int i=0; i<d; i++) //for all dimensions
            {
                ...
                if (i1_tmp*tmp_detB+eps_margin > 0)
                    c(k*d+i) = i1_tmp*tmp_detB + eps_margin;
        ...
        for (int k=0; k<K; k++)//for all states
        {
            ...
            eigVal.Sort();
            for (int i=0; i<d;i++)
                if (eigVal[i] + eps_margin>0)
                    c(k*d + i) = eigVal[i] + eps_margin;
...
bool SEDS::GMM_2_Parameters_MSE(Vector &p){ //this is used to transform GMM to the optimization parameters
    ...
    for (int k=0; k<K; k++){
        ...
        L_x[k] = Sigma_x[k].SCholesky();//set L_x
