...
public:
    ...
    */  
    ...
    inline Matrix& SetColumn(const REALTYPE value, const unsigned int col)
    {
        if(col<this->column){    
            REALTYPE *src = _ + col;
            unsigned int len = row; //by Mohammad
            while(len--){
                *src = value; 
                src += column;
            }
  ...
  Matrix& RemoveZeroColumns(){
    ...
    /// Sum each row and give the result in a vector
    Vector & SumRow(Vector & result){ //changed by Mohammad
        result.Resize(column,false);
        REALTYPE *src = _;
		unsigned int rowLen = row;
        while(rowLen--){
    ...
    /// Sum each column and give the result in a vector
    Vector & SumColumn(Vector & result){ //changed by Mohammad
        result.Resize(row,false);
        result.Zero();
        REALTYPE *src = _;
		REALTYPE *dst = result._;
    ...
    bool Save(const char* filename, unsigned int precision=6);
    /// Load a matrix from filename
        ...
        if((row!=rowSize)||(column!=colSize)){
            if((rowSize)&&(colSize)){
                ...
                if(copy){
                    ...
                    if(rowSize>row){
                        memset(arr+row*colSize*sizeof(REALTYPE),0,(rowSize-row)*colSize*sizeof(REALTYPE));
                    }
                }
                if(_!=NULL) delete [] _; 
                _      = arr;
...
protected:
    ...
    inline virtual SharedMatrix& Resize(unsigned int rowSize, unsigned int colSize, bool copy = true){
        assert((rowSize == row)&&(colSize=column));
        return *this;
    }
};
