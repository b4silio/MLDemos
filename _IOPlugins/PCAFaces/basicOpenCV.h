...
namespace BasicML
{
	...
	{
		if(!x || !y) return;
        if(dst == nullptr) dst = cvCreateImage(cvGetSize(x),8,1);
		...
		FOR(i,height){
			FOR(j, width){
                dst->imageData[i*width + j] =
                    isInsideEllipse(cvPoint2D32f((unsigned char)(x->imageData[i*width+j])/255.f, ((unsigned char)y->imageData[i*width+j])/255.f), eig, ratio) ?
                                                    (uchar)255 : 0;
			}
		}
	}
